# Marauders implementation history

Archived during the 2026-09-10 launch review. This preserves the preceding feature
checklist and verification history; it is not the active release plan. Use
[TASKLIST.md](TASKLIST.md) for current priorities and [DECISIONS.md](DECISIONS.md)
for subsequent scope changes, including the owner's open-access decision.
...
- [x] Correct Alyssa's display name to Alyssa the Sea Witch and read her renamed
  source portrait. Preserve her saved ID and web image URL. Client lint/build,
  solution build, all 133 server tests, and the five-browser character selection
  scenario pass; evidence is under ignored `artifacts/alyssa-name-e2e/`.

- [x] Follow-up: victory when only one captain owns ports; automatic random
  rebuilding of unused capacity at turn end; 135/45-second defaults with turn
  budgets scaling to starting action dice. Server and five-browser checks cover
  captures, forfeits, timeouts, manual/automatic builds, and restart persistence.
  See the September 10 follow-up decisions.
- [x] Follow-up maps: south entrance and northwest island on Serpent's Coil;
  one terrain island in each Choke bay, retaining its three-hex crossing.
  Fresh drafts use v5; v4 geometry retained for existing saves. Map, harbor,
  setup, sailing, synchronization, and all six legacy-layout checks pass.
...
- [x] September 10 map review: arrange The Choke's bay ports in circles, including
  ...
  Serpent's Heart, keeping the eastern breach and winding route. Retained in v5.
...
- [ ] Owner handles the old Marauders video; install it when supplied.
  The supplied personal character portraits are installed (see below).
...
- [x] Four-seat lobby, distinct colors and named character choices, host
...
- [x] September 9 geography pass: replace alternate-map symmetry and repeated
  ...
  September 10 concepts and v5 refinements supersede them for new games. See MAPS.md.
...
- [x] Install the eight supplied character names and portraits from Characters/.
  Generate optimized web copies, retain the originals, and include installed
  images in publish output. Require explicit character/color choices before
  joining, reject duplicates atomically, synchronize taken options, and release
  choices on lobby leave. Refresh/restart retains reservations.
...
- [x] Capture transfers ownership, voids construction, converts remaining ships
  after final-port loss, and declares victory when only one captain owns ports. Further play stops.
...
- [~] Playtest timer lengths and timeout casualty policy. Owner-approved defaults
  are now 135/45 seconds with turn budgets scaling to starting dice; the four-window practice override is
...
- September 10 character portraits: client lint/build, Debug solution build,
  all 133 server tests, same-origin publish, and all 13 browser scenarios pass.
  New five-browser coverage verifies all eight installed portraits and names,
  required explicit selections, invalid/missing inputs, simultaneous character
  claims, duplicate colors, live taken options, cross-tab identity, restart
  reservations, lobby leave/reclaim, and unique four-captain drafting. Server
  tests also race color claims and verify rematch/empty-reset behavior. Updated
  practice launcher and existing multiplayer scenarios use distinct characters.
  All eight images load from publish output. Lobby and draft screenshots were
  visually reviewed; evidence is in ignored artifacts/characters-e2e/ and
  artifacts/characters-e2e.log. Originals and existing saves were preserved.

- September 10 endgame/pacing/map follow-up: client lint/build, Debug solution
  build, all 122 server tests, and same-origin publish pass. Twelve browser
  scenarios pass across the regression run and focused reruns. The initial run
  exposed two outdated test assumptions (v4 as the latest map and shipless
  captains waiting despite full build queues); corrected fixtures pass.
  Coverage includes final capture with a neutral port, final forfeit with
  eliminated captains still seated, manual/automatic build choices, timeout
  rebuilding, five-die 4:30 budgets, 45-second action expiry, and persistence.
  Both v5 maps pass draft/launch/sailing, and all six v2/v3/v4 saves restore
  their own terrain in five browsers. Prior v4 files match the former committed
  geometry exactly. Map previews, actual browser maps, the construction hint,
  and victory history were visually reviewed. Rerun evidence is in ignored
  artifacts/followup-e2e/ and artifacts/followup-e2e.log; map previews are in
  artifacts/maps/. Existing live matches were not reset or restarted.
...
- Playtest feedback on perk power and quantity.
- Feedback on the 135/45-second defaults, fleet-sized turn budgets, and revised map balance.
